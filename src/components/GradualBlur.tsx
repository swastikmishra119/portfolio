...
function GradualBlur(props: GradualBlurProps) {
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      const p1 = Number(round((increment * i - increment) * 10)) / 10;
      const p2 = Number(round(increment * i * 10)) / 10;
      const p3 = Number(round((increment * i + increment) * 10)) / 10;
      const p4 = Number(round((increment * i + increment * 2) * 10)) / 10;
