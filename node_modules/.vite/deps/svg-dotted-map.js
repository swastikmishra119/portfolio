...
var V = (t) => {
  ...
  return { type: "Feature", id: "multipolygon", properties: { name: "Combined Polygons" }, geometry: { type: "MultiPolygon", coordinates: e } };
};
var U = ({ height: t = 0, width: e = 0, countries: o = [], rows: r, columns: a, region: p, radius: l = 0.3 }) => {
  let n = x;
  if (o.length > 0) {
    let g = X(n);
    n = { type: "FeatureCollection", features: o.map((i) => g[i]).filter((i) => i !== void 0) }, p || (p = H(n));
  } else p || (p = z);
  let y = { lat: { min: Math.max(p.lat.min, -85), max: Math.min(p.lat.max, 85) }, lng: p.lng }, u = V(n), [s, M] = C(y.lng.min, y.lat.min), [P, R] = C(y.lng.max, y.lat.max), F = P - s, m = R - M;
  ...
  for (let g = 0; g < G; g++) for (let i = 0; i < N; i++) {
    let B = d + i / (N - 1) * A, L = d + g / (G - 1) * S, h = [B / e * F + s, R - L / t * m], b = h[0] * 180 / 2003750834e-2, I = Math.atan(Math.exp(h[1] * Math.PI / 2003750834e-2)) * 360 / Math.PI - 90, f = Math.max(-180, Math.min(180, b)), E = Math.max(-85, Math.min(85, I));
    if (v([f, E], u)) {
      let Y = Math.round(i / N * (a - 1)), Z = Math.round(g / G * (r - 1)), K = d + Y / (a - 1) * A, J = d + Z / (r - 1) * S, w = `${Math.round(K)};${Math.round(J)}`;
      c[w] = { x: K, y: J };
    }
  }
  return { points: c, X_MIN: s, Y_MAX: R, X_RANGE: F, Y_RANGE: m, height: t, width: e };
};
var k = (t, e) => {
  let [o, r] = t, a = false;
...
var _ = (t, e) => {
  let o = e[0];
  if (!o || !k(t, o)) return false;
  for (let r = 1; r < e.length; r++) {
    let a = e[r];
    if (a && k(t, a)) return false;
  }
  return true;
};
var v = (t, e) => {
...
var r1 = ({ height: t, width: e, countries: o, region: r, radius: a = 0.3, mapSamples: p = 6e3 }) => {
  let l = e / t, n = Math.round(Math.sqrt(p / l)), y = Math.round(n * l), u = JSON.stringify({ height: t, width: e, countries: o, region: r, rows: n, columns: y, radius: a }), { points: s, X_MIN: M, Y_MAX: P, X_RANGE: R, Y_RANGE: F } = U({ height: t, width: e, countries: o, region: r, rows: n, columns: y, radius: a }), m = T.get(u);
  m || (m = U({ height: t, width: e, countries: o, region: r, rows: n, columns: y, radius: a }), T.set(u, m));
  let c = a * 1.25, d = e - 2 * c, A = t - 2 * c;
  return { points: Object.values(s), addMarkers: (S) => S.map((G) => {
...
//# sourceMappingURL=svg-dotted-map.js.map
